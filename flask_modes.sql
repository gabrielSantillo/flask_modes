...
INSERT INTO `painting` VALUES (1,'Test','1990-01-08','Test','testing');
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_artist`(artist_input varchar(100))
begin
	select p.id, p.name, p.date_painted 
	from painting p
...
artist_input varchar (100),
date_painted_input date,
image_url varchar(1000),
name_input varchar(100))
    MODIFIES SQL DATA
begin
	insert into painting (artist, date_painted, name, image_url)
	values (artist_input, date_painted_input, image_url, name_input);
...
-- Dump completed on 2022-10-11 19:24:48
